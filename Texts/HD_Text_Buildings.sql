...
values
    ...
    ("zh_Hans_CN",  "LOC_BUILDING_SANCTUARY_DESCRIPTION",                        "建成后，使本城的随机一种生物类资源在附近种群扩张。[NEWLINE][NEWLINE]本城的雨林、草本沼泽和泛滥单元格为周围单元格+1魅力。[NEWLINE][NEWLINE]本城的地貌单元格+1 [ICON_FOOD] 食物；若拥有资源，则额外+1 [ICON_PRODUCTION] 生产力；若位于迷人单元格，则额外+3 [ICON_GOLD] 金币；若位于惊艳单元格，则额外+1 [ICON_CULTURE] 文化值。[NEWLINE]本城的自然奇观单元格+2 [ICON_FOOD] 食物，+2 [ICON_CULTURE] 文化值和+6 [ICON_GOLD] 金币。"),
