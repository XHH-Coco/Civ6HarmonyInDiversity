...
values
    ...
    ("zh_Hans_CN",  "LOC_UNIT_SAPPER_DESCRIPTION",                                              "远古时期军事工程单位。[NEWLINE]移动时自动创建道路。可消耗1次劳动力加快运河、堤坝、水渠区域的20%的建造进度。可以在火山土上消耗1次劳动力进行“勘探”，有概率获得 [ICON_Gold] 金币或 [ICON_Science] 科技值，也有概率发现新资源或发掘出 [ICON_GREATWORK_RELIC] 遗物。一个单元格内单位回复生命值时+10每回合回复量。[NEWLINE]无视地形、地貌和河流的移动力消耗。"),
