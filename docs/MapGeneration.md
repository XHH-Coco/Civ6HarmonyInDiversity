...
结果**11 种可达，全部在北边界**。比只看方向搜索约束的理论值（13 种）少两种，
**少的正是南边界那两种**：`FLOWDIRECTION_SOUTH` 分支在 `y=0` 时要取 SW 邻格，
那格不存在，**会在铺边之前就 early return**，根本走不到搜索那一步。
所以南边界**结构上就不会产生断头河** —— 这比"没有实测样本"的说法强得多。

#### 关键区分：只有 `this == NORTH` 才是真的断在内陆

断头时"最后铺下的那条边"的**外侧有没有格子**，可以用 `Map.GetAdjacentPlot` 直接判，
不需要任何六边形顶点/角的几何知识。结果非常干净：

| `thisFlowDirection` | 分支铺的是 | 在 y=H−1 上外侧 | 含义 |
|---|---|---|---|
| `NORTH` | 某格的 **W 边** | **有格子** | 真的断在内陆，**需要补** |
| `NORTHEAST` / `SOUTHWEST` | riverPlot 的 **NW 边** | 没有 | 已经躺在地图上边界上 |
| `SOUTHEAST` | riverPlot 的 **NE 边** | 没有 | 同上 |
| `NORTHWEST` | 某格的 **NE 边** | 没有 | 同上 |

实测每一种都是 100%：7 种 `this=NORTH` 的组合末边贴图外 **0 次**，
另外 4 种 **全部**贴图外。

而且一条贴着边界的边，它**两个端点都在地图边界上**（边界边的两端各只被一两个格子
共享）。所以那 4 种情况下河已经"流出图外"了，**再补边纯属画蛇添足**。
原版补丁自己铺的就是一条 NW 边（边界边），这也印证了"河躺在边界边上 = 有出口"
正是 Firaxis 的本意。

> **这更正了我上一轮的说法**：我说"补丁只覆盖 8/11，剩下 3 种是漏网"，
> 那是错的。剩下 3 种不是漏网，是**不需要补**。

于是测试里的白名单可以扔掉，换成一条**完整的不变式**：

> 每一种可达的断头，要么被补丁补上，要么它最后铺下的那条边本来就贴着图外。

现在 **11 / 11 全部满足**：8 种靠补丁补上，3 种本来就贴边。

顺带发现：`this=NORTHWEST, orig=NORTHEAST` 这一种本来就贴着图外，
但保留下来的原版条件仍然会去补它，**多铺两条边**。无害，但确实是多余的 ——
说明原版那半条件从来没做过有用功。留着是为了不平白删除长期存在的行为；
真要收紧，条件可以简化成只判 `thisFlowDirection == FLOWDIRECTION_NORTH`
（变异测试里验证过，那是个等价实现，不会让不变式失败）。

#### 那"要不要给早停半格的河续一格"

**不用。** 上面那张表就是答案：会早停的那 4 种 `thisFlowDirection`，河的最后一条边
本来就贴着地图边界；真正停在内陆的只有 `this == NORTH` 一种，而它已经被补丁处理了。
"续一格"要解决的问题不存在。
...
四种人为破坏被抓到：退回原版条件、把 `NORTH` 写成 `SOUTH`、少铺一条边、
把 `NW` 写成 `NE`。

第五种"只留 `this` 那半条件"**预期不失败**，测试也确实没报错 ——
它是个等价实现（去掉的那半条件只会额外命中本来就贴边的情形）。
变异测试里显式标了它的预期，免得下次误以为是漏检。
