...
function GenerateMap()
    ...
    terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature,-3, 0, 0, 0.03, -0.03,0.01);
...
function GeneratePlotTypes(world_age)
    ...
    local sea_level = 1;
    local world_age = 1;    local iReefPercent = 18;
    ...
    elseif sea_level == 3 then -- Normal Sea Level
        water_percent_modifier = 0;
    elseif sea_level == 5 then -- High Sea Level
        water_percent_modifier = 4;
    else
        water_percent_modifier = TerrainBuilder.GetRandomNumber(6, "Random Sea Level - Lua") - 4;
    end
...
function AddFeatures()
    ...
    if rainfall == 5 then
        rainfall = 3 + TerrainBuilder.GetRandomNumber(5, "Random Rainfall - Lua");
    end
    
    local args = {rainfall = rainfall, iMarshPercent = 5, iSwampPercent = 3, iJunglePercent = 39, iForestPercent = 25} 
...
function GenerateFractalLayerWithoutHills (args, plotTypes)
    ...
    local iWaterPercent = args.iWaterPercent or 49;
...
function GenerateWaterLayer (args, plotTypes)
    ...
    -- Handle args or assign defaults.
    local iWaterPercent = args.iWaterPercent or 45;
    local iRegionWidth = args.iRegionWidth; -- Mandatory Parameter, no default
    local iRegionHeight = args.iRegionHeight; -- Mandatory Parameter, no default
    local iRegionWestX = args.iRegionWestX; -- Mandatory Parameter, no default
    local iRegionSouthY = args.iRegionSouthY; -- Mandatory Parameter, no default
...
function AddLakes(largeLakes)
    ...
    local lakePlotRand =  math.floor(GlobalParameters.LAKE_PLOT_RANDOM or 25 / 8);
    ...
    if(TerrainBuilder.CanHaveFeature(plot, g_FEATURE_REEF) and lat < self.iceLat * 0.9) then
        ...
        if(math.ceil(self.iReefCount * 100 / self.iNumReefablePlots) <= self.iReefMaxPercent) then
                ...
                if(TerrainBuilder.GetRandomNumber(200, "Resource Placement Score Adjust") >= iScore) then
                    ...
                    self.iReefCount = self.iReefCount / 2 + 1;
                end
        end
    end
end
