------------------------------------------------------------------------------
--	FILE:	 Forest_Highlands.lua  圭亚那高原
--	AUTHOR:  
...
function GenerateMap()
	print("Generating Forest_Highlands Map");
	local pPlot;
...
--------------------------------------------------------------------------
	...
	local numLargeLakes = math.ceil(GameInfo.Maps[Map.GetMapSize()].Continents * 5 + 0);
...
--	end
	...
	local args = {
		...
		ExtraBonusGroupOneMultiply = 1.4,							--增加的倍率
		ExtraBonusGroupOne = {	
			"RESOURCE_BANANAS",			--香蕉
			"RESOURCE_BANANAS",			--额外香蕉
...
function GeneratePlotTypes(world_age)
	...
	--local iWaterShift1 = 12
	--local iWaterHeight1 = g_iH - iWaterShift1 * 5;
	...
	-- by changing iRegionFracXExp and iRegionFracYExp to control the shape of oceans.
		...
		-- Generate Tiny Lakes
		local args = {};	
		args.iWaterPercent = 93 + water_percent_modifier;
		args.iRegionWidth = math.ceil(g_iW);
		args.iRegionHeight = math.ceil(g_iH);
		args.iRegionWestX = math.floor(0);
		args.iRegionSouthY = math.floor(0);
...
function AddFeatures()
	...
	if rainfall == 4 then
		...
		local args = {rainfall = rainfall, iMarshPercent = 3, iSwampPercent = 4, iJunglePercent = 39, iForestPercent = 23} 
	--雨林植被
...
function GenerateFractalLayerWithoutHills (args, plotTypes)
	...
	local iWaterPercent = args.iWaterPercent or 58;
	...
	if(iRiftGrain > 0 and iRiftGrain < 4) then
		local riftsFrac = Fractal.Create(g_iW, g_iH, rift_grain, {}, 3, 5);--改变了大陆视角大陆的分割形状以保护冻土文明可以出生到冻土边缘
	...
	for y = 0, iRegionHeight - 1, 1 do
		for x = 0, iRegionWidth - 1, 1 do
			...
			elseif (adjCount > 8 and adjCount < 15) then
				if (4 + TerrainBuilder.GetRandomNumber(2, "Fractal Layer Land - Lua") <= adjCount) then
					plotTypes2[i] = g_PLOT_TYPE_LAND;
				end
...
function AddTerrainFromContinents(plotTypes, terrainTypes, world_age, iW, iH, iContinentBoundaryPlots, bNoCoastalMountains)
	...
	local iMountainPercentByDistance:table = {15, 5, 2}; --可能表示在不同距离范围内生成山脉的概率。需测试
...
function FeatureGenerator:AddJunglesAtPlot(plot, iX, iY)
	...
	if(TerrainBuilder.CanHaveFeature(plot, g_FEATURE_JUNGLE)) then
		if(iY >= self.iJungleBottom  and iY <= self.iJungleTop) then 
			self.iNumJunglablePlots = self.iNumJunglablePlots + 2.05;
			if(math.ceil(self.iJungleCount * 100 / self.iNumJunglablePlots) <= self.iJungleMaxPercent) then
				...
				if(TerrainBuilder.GetRandomNumber(600, "Resource Placement Score Adjust") <= iScore) then
					...
					self.iJungleCount = self.iJungleCount + 1.5;
					return true;
				end
			end
		end
