...
function AssignStartingPlots:__GetValidAdjacent(plot, minor)
    ...
    if (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_CANADA") or (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_RUSSIA")then
        max = math.ceil(gridHeight * self.uiStartMaxY / 100);--与409行相关
        min = math.ceil(gridHeight * self.uiStartMinY / 100);--与409行相关，409行设置1.14时此处不宜大于130
    end
...
function AssignStartingPlots:__NaturalWonderBuffer(plot, minor)
    ...
    if(minor == true) then
        ...
        iMaxNW = GlobalParameters.START_DISTANCE_MAJOR_NATURAL_WONDER or 6;
    end
...
function AssignStartingPlots:__LuxuryBuffer(plot)
    ...
    for dx = -2, 2 do
        for dy = -2,2 do
            local anotherPlotIndex = (plotX + dx) + (plotY + dy) * gridWidth;
            local distance = Map.GetPlotDistance(plot, anotherPlotIndex);
            local anotherPlot = Map.GetPlotByIndex(anotherPlotIndex);
            if 1 <= distance and distance <= 2 and anotherPlot ~= nil then
                if(anotherPlot:GetResourceCount() > 0) then
                    for row = 0, iResourcesInDB do
                        if (eResourceClassType[row]== "RESOURCECLASS_LUXURY") then
                            if(eResourceType[row] == anotherPlot:GetResourceType()) then
                                return true;
                            end
                        end
                    end
...
function AssignStartingPlots:__MajorCivBuffer(plot)
    ...
    local iMaxStart = GlobalParameters.START_DISTANCE_MAJOR_CIVILIZATION or 11;

    if (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_CANADA") or (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_RUSSIA")then
        local iMaxStart = GlobalParameters.START_DISTANCE_MAJOR_CIVILIZATION or 13;
    end
...
function AssignStartingPlots:__MinorMajorCivBuffer(plot)
    ...
    if (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_CANADA") or (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_RUSSIA")then
        local iMaxStart = GlobalParameters.START_DISTANCE_MINOR_MAJOR_CIVILIZATION -1 or 9;
    end
    local iSourceIndex = plot:GetIndex();
...
function AssignStartingPlots:__MinorMinorCivBuffer(plot)
    ...
    local iMaxStart = GlobalParameters.START_DISTANCE_MINOR_CIVILIZATION_START or 4;

    if (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_CANADA") or (PlayerConfigurations[0]:GetCivilizationTypeName() == "CIVILIZATION_RUSSIA")then
        local iMaxStart = GlobalParameters.START_DISTANCE_MINOR_CIVILIZATION_START - 1 or 7;
    end
    local iSourceIndex = plot:GetIndex();
...
function AssignStartingPlots:__StartBiasResources(playerIndex, tier, minor)
    ...
    if(minor == true) then
     ...
     if  (civilizationType == "CIVILIZATION_CANADA")
        or  (civilizationType == "CIVILIZATION_RUSSIA")then
        range = 1 or 4;
    end
    resourcePlots = {};
...
function AssignStartingPlots:__StartBiasFeatures(playerIndex, tier, minor)
    ...
    --试验1环1.3.9 by txym 
    if  (civilizationType == "CIVILIZATION_BRAZIL")
        or  (civilizationType == "CIVILIZATION_EGYPT")
        or  (civilizationType == "CIVILIZATION_KONGO") then
        range = 1;
    end
    if  (civilizationType == "CIVILIZATION_SPAIN")
        or  (civilizationType == "CIVILIZATION_NUBIA") 
        or  (civilizationType == "CIVILIZATION_POLAND") 
        or  (civilizationType == "CIVILIZATION_HUNGARY") then
        range = 2 or 4;
    end    
    featurePlots = {};
...
function AssignStartingPlots:__StartBiasTerrains(playerIndex, tier, minor)
    ...
    --1.3.9 by txym 
    if  (civilizationType == "CIVILIZATION_THIOPIA")then
        range = 1;
    end

    if  (civilizationType == "CIVILIZATION_NUBIA") 
        or  (civilizationType == "CIVILIZATION_MAPUCHE")
        or  (civilizationType == "CIVILIZATION_INCA")then
        range = 2;
    end
     if  (civilizationType == "CIVILIZATION_CANADA")
        or  (civilizationType == "CIVILIZATION_RUSSIA")then
