...
function GenerateMap()
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature,false, -3); -- Remove Coastal Mountains.
...
--	end
	...
	local args = {
		...
		MAX_MAJOR_CIV_FERTILITY = 141,
        MIN_MINOR_CIV_FERTILITY = 25, 
        MAX_MINOR_CIV_FERTILITY = 79,
...
function AddFeatures()
	...
	--植被
		local args = {rainfall = rainfall, iMarshPercent = 3, iSwampPercent = 3, iJunglePercent = 26, iForestPercent = 20} 
...
function GenerateFractalLayerWithoutHills (args, plotTypes)
	...
	for y = 0, iRegionHeight - 1, 1 do
		for x = 0, iRegionWidth - 1, 1 do
			...
			elseif (adjCount > 12 and adjCount <= 18) then
				if (3 + TerrainBuilder.GetRandomNumber(2, "Fractal Layer Land - Lua") <= adjCount) then
					plotTypes2[i] = g_PLOT_TYPE_LAND;
				end
