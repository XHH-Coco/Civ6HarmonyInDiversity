...
function GenerateMap()
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature, false, -7); -- Remove Coastal Mountains.
	...
	local numLargeLakes = math.ceil(GameInfo.Maps[Map.GetMapSize()].Continents * 6);
...
--	end
	...
	local args = {
		...
		FewerBonusGroupOneMultiply = 0.75,							--减少的倍率
		FewerBonusGroupOne = {	
	...
	local args = {
		...
		MAX_MAJOR_CIV_FERTILITY = 166,
        MIN_MINOR_CIV_FERTILITY = 35, 
        MAX_MINOR_CIV_FERTILITY = 70,
...
function GeneratePlotTypes(world_age)
	...
	local mountainRatio = 6 + world_age * 5 ; --further lower the mountainRation in order to generate less lonely mountains
...
function AddFeatures()
	...
	local args = {rainfall = rainfall, iMarshPercent = 8, iSwampPercent = 7, iJunglePercent = 30, iForestPercent = 22, iOasisPercent = 22}
...
function GenerateFractalLayerWithoutHills (args, plotTypes)
	...
	for y = 0, iRegionHeight - 1, 1 do
		for x = 0, iRegionWidth - 1, 1 do
			...
			elseif (adjCount > 13 and adjCount < 33) then
				if (5 + TerrainBuilder.GetRandomNumber(7, "Fractal Layer Land - Lua") <= adjCount) then
					plotTypes2[i] = g_PLOT_TYPE_LAND;
				end
