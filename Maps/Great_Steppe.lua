...
function GenerateMap()
	print("Generating Great Steppe Map");
	local pPlot;
...
--------------------------------------------------------------------------
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature, false, -3, 0, 0.03, 0.04,0.03);
	...
	local numLargeLakes = math.ceil(GameInfo.Maps[Map.GetMapSize()].Continents * 7 + 4);
	...
	--local iMaxFloodplainSize = 4;
	MarkCoastalLowlands(1.5);
...
--	end
	...
	local args = {
		...
		HorsesMultiply =    1.8,	--调整马资源丰度
        IronMultiply =		1.2,	--调整铁资源丰度
        NiterMultiply =     0.7,	--调整硝石资源丰度
        CoalMultiply =      0.6,	--调整煤炭资源丰度
        OilMultiply =       0.7,	--调整石油资源丰度
        AluminumMultiply =  0.7,	--调整铝资源丰度

		ExtraBonusGroupOneMultiply = 1.6,							--增加的倍率
		ExtraBonusGroupOne = {	
			"RESOURCE_SHEEP",			--羊
			"RESOURCE_HAM",				--火腿
			"RESOURCE_CATTLE"			--牛		
		...
		ExtraBonusGroupTwoMultiply = 1.25,							--增加的倍率
		ExtraBonusGroupTwo = {	
			"RESOURCE_WHEAT",			--小麦
			"RESOURCE_MAIZE",			--玉米
			"RESOURCE_BARLEY",			--青稞
		...
		FewerBonusGroupTwoMultiply = 0.66,							--减少的倍率
		FewerBonusGroupTwo = {	
			"RESOURCE_TIN",				--锡矿
			"RESOURCE_PINE",			--松木
			"RESOURCE_OAK",				--橡木	
		...
		ExtraLuxuries = {	
			"RESOURCE_SORGHUM",				--高粱
			"RESOURCE_LEU_P0K_LLAMAS",		--大羊驼
			"RESOURCE_CASHMERE",			--羊绒
			"RESOURCE_IVORY",				--象牙
			"RESOURCE_LION",				--狮子
			"RESOURCE_SUK_CHEESE"			--奶酪			
		}--增加陆地奢，奢侈按大陆加无倍率
	...
	local args = {
		...
		MAX_MAJOR_CIV_FERTILITY = 155,
        MIN_MINOR_CIV_FERTILITY = 22, 
        MAX_MINOR_CIV_FERTILITY = 77,
...
function GeneratePlotTypes(world_age)
	...
	local sea_level_normal = 55;
	local sea_level_high = 77;
...
--------------------------------------------
	...
	local hillsBottom1 = 46 - adjustment;
	local hillsTop1 = 33 + adjustment;
	local hillsBottom2 = 75 - adjustment;
	local hillsTop2 = 21 + adjustment;
	local hillsClumps = 44 + adjustment*1.6;
	local hillsNearMountains = 60 - (adjustment * 1.5) - extra_mountains* 0.5;
	...
	AreaBuilder.Recalculate();
	--local iWaterShift1 = math.ceil(g_iH * 66 / 100);
...
--------------------------------------------
	...
	local args = {};	
	args.iWaterPercent = 71 + water_percent_modifier;
...
function AddFeatures()
	...
	if rainfall == 4 then
		...
		local args = {rainfall = rainfall, iMarshPercent = 4, iSwampPercent = 2, iJunglePercent = 13, iForestPercent = 8} 
	--稀疏的森林植被
...
function GenerateWaterLayer (args, plotTypes)
    ...
    -- Handle args or assign defaults.
    local iWaterPercent = args.iWaterPercent or 46;
    local iRegionWidth = args.iRegionWidth; -- Mandatory Parameter, no default
    local iRegionHeight = args.iRegionHeight; -- Mandatory Parameter, no default
    local iRegionWestX = args.iRegionWestX; -- Mandatory Parameter, no default
    local iRegionSouthY = args.iRegionSouthY; -- Mandatory Parameter, no default
...
function MarkCoastalLowlands()
	...
	local numDesiredCoastalLowlandsPercentage = 66;
