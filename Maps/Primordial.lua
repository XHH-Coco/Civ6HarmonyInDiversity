...
function GenerateMap()
	print("Generating Primordial Map"); --原始重塑
	local pPlot;
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature, 0, 0, 6, 0.05, 0.1, -0.05, 0.0, 0.0);
	...
	local args = {
        ...
        MAX_MAJOR_CIV_FERTILITY = 145,    
        MIN_MINOR_CIV_FERTILITY = 20, 
        MAX_MINOR_CIV_FERTILITY = 62, 
...
function AddFeatures()
	...
	local args = {rainfall = rainfall, iMarshPercent = 5,  iSwampPercent = 5, iJunglePercent = 55, iForestPercent = 25, iIcePercent = -7}
                    ...
                    for _, pNeighborPlot in ipairs(tNeighborPlots) do
                        ...
                        for _, rNeighborPlot in ipairs(sNeighborPlots) do
                            if (CanTakeVolcanicSoil(rNeighborPlot)) then
                                if (TerrainBuilder.GetRandomNumber(2, "Volcanic Soil - Lua") == 0) then
