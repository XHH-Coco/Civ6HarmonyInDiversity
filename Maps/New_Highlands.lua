...
--------------------------------------------------------------------------
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, true, temperature, false, -3, 0, 0.03, 0.03,0.01);
...
--	end
	...
	local args = {
		...
		HorsesAndIronMultiply = 1.5,	--马铁倍率

		ExtraBonusGroupOneMultiply = 3.2,							--增加的倍率
		ExtraBonusGroupOne = {	
			"RESOURCE_MAIZE",			--玉米
		...
		ExtraBonusGroupTwoMultiply = 1.65,							--增加的倍率
		ExtraBonusGroupTwo = {	
			"RESOURCE_COPPER",			--铜矿
			"RESOURCE_WHEAT",			--小麦
			"RESOURCE_BERRIES",			--浆果	
		...
		FewerBonusGroupTwoMultiply = 0.4,							--减少的倍率
		FewerBonusGroupTwo = {	
...
function GeneratePlotTypes(world_age)
	...
	local sea_level_normal = 48;
	local sea_level_high = 61;
	--local world_age = 3;
...
--------------------------------------------
	...
	local hillsClumps = 55 + adjustment*1.6;
	local hillsNearMountains = 95 - (adjustment * 1.0) - extra_mountains;
	...
	mountainsFrac:BuildRidges(numPlates * 3, g_iFlags, 6, 1);
...
--------------------------------------------
	...
	mountainRatio = 3 + world_age * 9 / 13 ;
...
function AddFeatures()
	...
	if rainfall == 4 then
		...
		local args = {rainfall = rainfall, iMarshPercent = 3, iSwampPercent = 2, iJunglePercent = 16, iForestPercent = 10} 
	--稀疏的森林植被
