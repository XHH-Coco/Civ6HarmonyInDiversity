...
function GenerateMap()
	...
	terrainTypes = GenerateTerrainTypes(plotTypes, g_iW, g_iH, g_iFlags, false, temperature, false, -8, 0, 0.03, -0.03,0,0.08,-0.12); -- Remove Coastal Mountains →保留沿海小山
	...
	AddTerrainFromContinents(plotTypes, terrainTypes, world_age, g_iW, g_iH, iContinentBoundaryPlots, true); -- 去除了大陆边缘山脉
...
--	end
	...
	local args = {
		...
		ExtraBonusGroupOneMultiply = 2.16,							--增加的倍率
		ExtraBonusGroupOne = {	
...
function GeneratePlotTypes(world_age)
 ...
 --    plotTypes = GenerateFractalLayerWithoutHills(args, plotTypes);
	...
	-- 方向跟随原版：年轻世界造山带多、孤峰少。
	mountainRatio = (15 + world_age_base * 6) * (1 + g_iWorldAgeStep * 0.2);
...
function GenerateFractalLayerWithoutHills (args, plotTypes)
	...
	local iWaterPercent = args.iWaterPercent or 75;
