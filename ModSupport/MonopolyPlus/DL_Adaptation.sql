...
-- 基础产出
update Improvement_YieldChanges set YieldChange = 6 where ImprovementType = 'IMPROVEMENT_LEU_STATION' and YieldType = 'YIELD_PRODUCTION';
insert or replace into Improvement_YieldChanges (ImprovementType, YieldType, YieldChange) values
	('IMPROVEMENT_LEU_STATION', 'YIELD_GOLD', 9);

-- 相邻加成
delete from Improvement_Adjacencies where ImprovementType = 'IMPROVEMENT_LEU_STATION' and YieldChangeId in (
	'Station_Production_From_Industry',
	'Station_Production_From_Corporation',
	'Station_Production_From_Districts',
	'Station_Production_From_Wonder'
);
update Adjacency_YieldChanges set PrereqTech = NULL where ID = 'Station_Culture_From_Wonder';
...
insert or ignore into ModifierArguments (ModifierId, Name, Value) values
	('LEU_STATION_IMPROVEMENT_TOURISM', 'Amount', 100);
