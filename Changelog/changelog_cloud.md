...
`RiversLakes.lua`，桩掉十来个引擎接口。

**先说清楚它测的不是任何一张真实地图**：地形是合成的（白噪声 + 做过平滑的连片陆地），
不是 `GeneratePlotTypes` 生成的分形大陆；起河也是直接调 `DoRiver`，没走 `AddRivers`
的选源逻辑。分形生成器和 `GetInlandCorner` 都在引擎里，游戏外复现不了。
所以「每张图会出现几条断头河」离线测不出来，**而且各图一定不一样** ——
断头要求首行/末行是陆地，极地是海还是雪原、河源密不密，每张图差很多。

离线能回答的是另一个更重要的问题：**哪些情况会发生、补丁覆盖了几种**，
这个只取决于 `DoRiver` 的控制流，与地形无关。

随机采样（4 个场景 × 3 种尺寸 × 12 张，起河 89742 次、232634 条河流边）：

```
走到 NO_FLOWDIRECTION（断在陆地上）：80 次
  按位置：北边界 y=H-1  80        ← 内陆 0、南边界 0
  按组合：this=NORTH orig=NORTHEAST 62 / orig=NORTHWEST 16 / orig=NORTH 2
```

`thisFlowDirection` **全部**是 `NORTH`。

但随机采样只能说「实际碰到了哪几种」，回答不了「还有哪几种到得了」。所以另加了一节
**定向穷举**：在全陆地图上用每个格子 × 每一组 `(this, original)` 直接调 `DoRiver`。
结果 **11 种可达、全在北边界**，比只看方向搜索约束算出的 13 种少两种 ——
少的正是南边界那两种：`FLOWDIRECTION_SOUTH` 分支在 `y=0` 要取 SW 邻格，那格不存在，
**会在铺边之前 early return**。所以**南边界结构上就不会产生断头河**，
这比「没有实测样本」强得多。
...
结果随机采样里 **80 次断头全部补上、未补 0 次**。

定向穷举里补丁只命中 11 种里的 8 种，但**剩下 3 种不是漏网，是不需要补**。
判据不需要任何六边形几何：看断头时最后铺下的那条边，外侧有没有格子。

| `thisFlowDirection` | 铺的是 | y=H−1 上外侧 | 含义 |
|---|---|---|---|
| `NORTH` | 某格的 **W 边** | **有格子** | 真断在内陆，需要补 |
| 其余四种 | riverPlot 的 **NW / NE 边** | 没有 | 已经躺在地图边界上 |

实测每种都是 100%。而边界边的**两个端点都在地图边界上**，所以那几种情况河
已经"流出图外"了。原版补丁自己铺的就是一条 NW 边，也印证了这就是 Firaxis 的本意。

于是白名单扔掉，换成一条**完整的不变式**：每一种可达的断头，要么被补上，
要么最后铺下的那条边本来就贴着图外 —— **11 / 11 全部满足**。

顺带发现：保留下来的原版半条件其实**从没做过有用功** ——
它只会额外命中 `this=NORTHWEST, orig=NORTHEAST`，而那种情况本来就贴边，
补上去等于多铺两条边。留着只是为了不平白删掉长期存在的行为。

顺带在断头处加了一句诊断 `print("RIVER DEAD END: ...")`，游戏里也能用，
可以核对实机分布是否和离线一致。

**没做**：「撞上已有河流提前 return」那一类不在边界上，补边界解决不了，
需要单独判断两条河是否真的接上。

想要真实的每图数字只能进游戏：断头处那句 `RIVER DEAD END` 会进 `Logs/Lua.log`，
25 张图各开几局 grep 一下即可。
...
两条边（靠铺边事件的先后顺序判断）。四种人为变异被抓到（退回原版条件、NORTH 写成 SOUTH、少铺一条边、NW 写成 NE）。
第五种「只留 this 那半条件」**预期不失败**，因为它是等价实现；
变异测试里显式标了预期，免得下次误以为是漏检。
