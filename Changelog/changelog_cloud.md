...
结果随机采样里 **80 次断头全部补上、未补 0 次**，定向穷举里覆盖 **8 / 11 种**。

**剩下 3 种刻意不补**（`this` 是 NORTHEAST / SOUTHEAST / SOUTHWEST）：
补的那两条边在几何上只对应「以 NORTH 到达」，那三种情况河停在这一格的另一个角上，
硬套同一组边多半会铺出一段接不上的孤边，比留着更糟。它们在 89742 次随机起河里
一次都没出现。测试用**白名单**钉住：新出现补不上的组合会失败，白名单里的组合
变成能补也会失败（说明白名单过期）。
...
两条边（靠铺边事件的先后顺序判断）。五种人为变异全部被抓到（含「只留 this 那半条件」—— 它会让
`this=NORTHWEST, orig=NORTHEAST` 失去覆盖，被白名单过期检查抓住）。
