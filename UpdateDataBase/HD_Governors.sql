...
insert or replace into HD_Binary_Compress_Keys (Key, MaxExp) values
	...
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_FOOD',        10),
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_PRODUCTION',  10),
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_SCIENCE',     10),
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_CULTURE',     10),
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_GOLD',        10),
	('HD_PLOT_BINARY_COMPRESS_GOVERNOR_MERCHANT_RIGHT_3_YIELD_FAITH',       10);
...
insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_EDUCATOR_RIGHT_3_CITY_YIELDS_' || Exp, 'YieldType', 'YIELD_SCIENCE, YIELD_CULTURE'
from HD_Binary_Compress where Exp < 6;

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_EDUCATOR_RIGHT_3_CITY_YIELDS_' || Exp, 'Amount', (Amount * 5) || ', ' || (Amount * 5)
from HD_Binary_Compress where Exp < 6;
...
insert or ignore into Modifiers (ModifierId, ModifierType, SubjectRequirementSetId) select
  'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType, 'MODIFIER_PLAYER_DISTRICTS_ADJUST_YIELD_MODIFIER', 'HD_DISTRICT_IS_' || DistrictType || '_WITHIN_4_TILES_REQUIREMENTS'
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;
...
insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType, 'Amount', 50
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;

insert or ignore into GovernorPromotionModifiers (GovernorPromotionType, ModifierId) select
  'GOVERNOR_PROMOTION_HD_BUILDER_LEFT_2', 'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType || '_LATE'
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;

insert or ignore into Modifiers (ModifierId, ModifierType, OwnerRequirementSetId, SubjectRequirementSetId) select
  'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType || '_LATE', 'MODIFIER_PLAYER_DISTRICTS_ADJUST_YIELD_MODIFIER', 'PLAYER_HAS_TECH_ARCH_HD_REQUIREMENTS', 'HD_DISTRICT_IS_' || DistrictType || '_WITHIN_4_TILES_REQUIREMENTS'
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType || '_LATE', 'YieldType', YieldType
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_BUILDER_LEFT_2_' || DistrictType || '_LATE', 'Amount', 50
from DistrictCorrespondingYieldType_HD where HasAdjacency = 1;
...
insert or ignore into GovernorPromotionModifiers (GovernorPromotionType, ModifierId) values
  ...
  ('GOVERNOR_PROMOTION_HD_MANAGER_LEFT_1',   'HD_GOVERNOR_MANAGER_LEFT_1_REGIONAL_FOOD'),
  ('GOVERNOR_PROMOTION_HD_MANAGER_LEFT_1',   'HD_GOVERNOR_MANAGER_LEFT_1_REGIONAL_PRODUCTION'),
  ('GOVERNOR_PROMOTION_HD_MANAGER_LEFT_1',   'HD_GOVERNOR_MANAGER_LEFT_1_REGIONAL_AMENITY'),
...
-- 横向一体化
insert or ignore into GovernorPromotionModifiers (GovernorPromotionType, ModifierId) select
  'GOVERNOR_PROMOTION_HD_MANAGER_LEFT_3', 'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType || '_ATTACH'
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into Modifiers (ModifierId, ModifierType, SubjectRequirementSetId) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType || '_ATTACH', 'MODIFIER_CITY_DISTRICTS_ATTACH_MODIFIER', 'REQUIRES_DISTRICT_IS_' || DistrictType || '_UDMET'
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType || '_ATTACH', 'ModifierId', 'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into Modifiers (ModifierId, ModifierType) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType, 'MODIFIER_SINGLE_CITY_ADJUST_TRADE_ROUTE_YIELD_TO_OTHERS'
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType, 'YieldType', YieldType
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType, 'Amount', YieldChangeAsDomesticDestination
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);

insert or ignore into ModifierArguments (ModifierId, Name, Value) select
  'HD_GOVERNOR_MANAGER_LEFT_3_TRADE_' || DistrictType || '_' || YieldType, 'Domestic', 1
from District_TradeRouteYields where YieldChangeAsDomesticDestination != 0 and DistrictType not in (select CivUniqueDistrictType from DistrictReplaces);
...
-- ============================================================================================================================================================
insert or ignore into GovernorPromotionModifiers (GovernorPromotionType, ModifierId) values
  ('GOVERNOR_PROMOTION_HD_MERCHANT_BASE',     'HD_GOVERNOR_MERCHANT_BASE_COMMERCIAL_HUB_ADJACENCY_BONUS'),
  ('GOVERNOR_PROMOTION_HD_MERCHANT_BASE',     'HD_GOVERNOR_MERCHANT_BASE_HARBOR_ADJACENCY_BONUS'),
  ...
  ('GOVERNOR_PROMOTION_HD_MERCHANT_RIGHT_1',  'HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_1_TRADE_GOLD'),
  ('GOVERNOR_PROMOTION_HD_MERCHANT_RIGHT_1',  'HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_2_TRADE_GOLD'),
  ('GOVERNOR_PROMOTION_HD_MERCHANT_RIGHT_1',  'HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_3_TRADE_GOLD'),
  ('GOVERNOR_PROMOTION_HD_MERCHANT_RIGHT_1',  'HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_4_TRADE_GOLD'),
  ('GOVERNOR_PROMOTION_HD_MERCHANT_RIGHT_2',  'HD_GOVERNOR_MERCHANT_RIGHT_2_TRADE_ROUTE'),
...
insert or ignore into Modifiers (ModifierId, ModifierType, SubjectRequirementSetId) values
  ('HD_GOVERNOR_MERCHANT_BASE_COMMERCIAL_HUB_ADJACENCY_BONUS',          'MODIFIER_CITY_DISTRICTS_ADJUST_YIELD_MODIFIER',                        'DISTRICT_IS_COMMERCIAL_HUB'),
  ('HD_GOVERNOR_MERCHANT_BASE_HARBOR_ADJACENCY_BONUS',                  'MODIFIER_CITY_DISTRICTS_ADJUST_YIELD_MODIFIER',                        'DISTRICT_IS_HARBOR'),
  ...
  ('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_1_TRADE_GOLD',     'MODIFIER_SINGLE_CITY_ADJUST_TRADE_ROUTE_YIELD_FOR_INTERNATIONAL',      'CITY_HAS_DISTRICT_COMMERCIAL_HUB_TIER_1_BUILDING_REQUIREMENTS'),
  ('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_2_TRADE_GOLD',     'MODIFIER_SINGLE_CITY_ADJUST_TRADE_ROUTE_YIELD_FOR_INTERNATIONAL',      'CITY_HAS_DISTRICT_COMMERCIAL_HUB_TIER_2_BUILDING_REQUIREMENTS'),
  ('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_3_TRADE_GOLD',     'MODIFIER_SINGLE_CITY_ADJUST_TRADE_ROUTE_YIELD_FOR_INTERNATIONAL',      'CITY_HAS_DISTRICT_COMMERCIAL_HUB_TIER_3_BUILDING_REQUIREMENTS'),
  ('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_4_TRADE_GOLD',     'MODIFIER_SINGLE_CITY_ADJUST_TRADE_ROUTE_YIELD_FOR_INTERNATIONAL',      'CITY_HAS_DISTRICT_COMMERCIAL_HUB_TIER_4_BUILDING_REQUIREMENTS'),
  ('HD_GOVERNOR_MERCHANT_RIGHT_2_TRADE_ROUTE',                          'MODIFIER_PLAYER_ADJUST_TRADE_ROUTE_CAPACITY',                          NULL),
...
insert or ignore into ModifierArguments (ModifierId, Name, Value) values
	('HD_GOVERNOR_MERCHANT_BASE_COMMERCIAL_HUB_ADJACENCY_BONUS',          'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_BASE_COMMERCIAL_HUB_ADJACENCY_BONUS',          'Amount',                 100),
	('HD_GOVERNOR_MERCHANT_BASE_HARBOR_ADJACENCY_BONUS',                  'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_BASE_HARBOR_ADJACENCY_BONUS',                  'Amount',                 100),
	...
	('HD_GOVERNOR_MERCHANT_RIGHT_1_TRADE_ROUTE',                          'Amount',                 1),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_1_TRADE_GOLD',     'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_1_TRADE_GOLD',     'Amount',                 3),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_2_TRADE_GOLD',     'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_2_TRADE_GOLD',     'Amount',                 3),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_3_TRADE_GOLD',     'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_3_TRADE_GOLD',     'Amount',                 3),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_4_TRADE_GOLD',     'YieldType',              'YIELD_GOLD'),
	('HD_GOVERNOR_MERCHANT_RIGHT_1_COMMERCIAL_HUB_TIER_4_TRADE_GOLD',     'Amount',                 3),
	('HD_GOVERNOR_MERCHANT_RIGHT_2_TRADE_ROUTE',                          'Amount',                 1),
...
from Yields, HD_Binary_Compress where Exp < 11;
