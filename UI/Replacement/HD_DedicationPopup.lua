...
function CreateCommemoration(commemorationInfo:table)
  ...
  local canSelected = localPlayer:GetProperty(COMMEMORATION_CANNOT_SELECT_TAG .. commemorationInfo.Index) or 0
  ...
  -- 如果不可选择该着力点，修改文本
  if canSelected == 1 then
    bonusText = Locale.Lookup('LOC_COMMEMORATION_CANNOT_SELECT') .. bonusText
    -- 添加悬浮框提示文本
    local reason = localPlayer:GetProperty(COMMEMORATION_CANNOT_SELECT_REASON_TAG .. commemorationInfo.Index)
    if reason ~= nil then
...
function OnCommemorationSelected(selectedInstance:table, commemorationType:number)
  ...
  local localPlayerID:number = Game.GetLocalPlayer();
  local localPlayer = Players[localPlayerID]
  if localPlayer:GetProperty(COMMEMORATION_CANNOT_SELECT_TAG .. commemorationType) == 1 then
    return
  end

	-- How many are we allowed to select?
	local selectionsAllowed:number = Game.GetEras():GetPlayerNumAllowedCommemorations(localPlayerID);
