print("HD Override TechAndCivicSupport");
	...
	if(unlockables and #unlockables > 0) then
		for i,v in ipairs(unlockables) do
			...
			if(kGovernmentIM and typeInfo and typeInfo.Kind == "KIND_GOVERNMENT") then
				...
				-- 隐藏总督点图标
				if unlockIcon.TechUnlockNumberedContainer then
					unlockIcon.TechUnlockNumberedContainer:SetHide(true);
				end
...
--
-- ===========================================================================
function PopulateUnlockablesForTech(playerID:number, techID:number, instanceManager:table, callback:ifunction, hideExtraIcons:boolean )
  ...
  -- HD 新增科技树解锁总督点
	if not hideExtraIcons then
		local unlockGovernor = GameInfo.HD_TechnologyGovernorPoints[techType];
		-- print('科技树解锁总督点', techType, unlockGovernor)
		if unlockGovernor then
			local unlockIcon:table	= instanceManager:GetInstance();
			unlockIcon.Icon:SetHide(true); -- foreground icon unnecessary in this case
			local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas("ICON_HD_BLANK",38);
			if textureSheet ~= nil then
				unlockIcon.UnlockIcon:SetTexture(textureOffsetX, textureOffsetY, textureSheet);
			end
			unlockIcon.UnlockIcon:LocalizeAndSetToolTip(nil);
			if unlockIcon.TechUnlockNumberedContainer then
				unlockIcon.TechUnlockNumberedContainer:SetHide(false);
				unlockIcon.FontIcon:SetText("[Icon_Governor]");
				unlockIcon.TechUnlockNumberedIcon:SetToolTipString(Locale.Lookup("LOC_HUD_CIVICS_TREE_AWARD_GOVERNOR"));
				unlockIcon.NumberLabel:SetText(tostring(1));
				numIcons = numIcons + 1;
			end
		end
	end
