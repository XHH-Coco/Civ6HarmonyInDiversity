...
local m_ReqLinesLeftLongIM:table = InstanceManager:new("ReqLinesLeftLong", "Top");
local m_ReqLinesRightLongIM:table = InstanceManager:new("ReqLinesRightLong", "Top");
...
function CreatePromotionTree(localPlayerID:number, pGovernor:table, pGovernorDef:table, pPlayerGovernors:table)
	...
	for promotionSet in GameInfo.GovernorPromotionSets() do
		if promotionSet.GovernorType == pGovernorDef.GovernorType then
			...
			if kPromotion.BaseAbility then
				...
				pPromotionTreeInst.BaseAbilityLabel:SetText(sName .. Locale.Lookup('LOC_TOOLTIP_HD_COLON_TEXT') .. sDescription);
			else
			...
			-- HD记录升级信息
			promotionMap[kPromotion.GovernorPromotionType] = {
				Column = kPromotion.Column,
				Level = kPromotion.Level
			}
		end
	end
	...
	m_ReqLinesLeftLongIM:ResetInstances();
	m_ReqLinesRightLongIM:ResetInstances();

	-- HD依照GovernorPromotionPrereqs表绘制前置关系箭头
	for row in GameInfo.GovernorPromotionPrereqs() do
		local governorPromotionType = row.GovernorPromotionType;
		local governorPromotionTypeData = promotionMap[governorPromotionType];

		local prereqGovernorPromotion = row.PrereqGovernorPromotion;
		local prereqGovernorPromotionData = promotionMap[prereqGovernorPromotion];
		if governorPromotionTypeData and prereqGovernorPromotionData
			and governorPromotionTypeData.Level - prereqGovernorPromotionData.Level == 1
		then
			local x = prereqGovernorPromotionData.Column + 1;
			local y = prereqGovernorPromotionData.Level;

			local xOffset = PROMOTION_X_OFFSET * (x - 1);
			local yOffset = (PROMOTION_Y_OFFSET * (y - 1)) + PROMOTION_LINE_Y_OFFSET;

			-- 判断连接形式
			local columnOffset = governorPromotionTypeData.Column - prereqGovernorPromotionData.Column;
			...
			elseif columnOffset == -1 then
				...
				local adjustedXOffset:number = x ~= 2 and xOffset or xOffset - EXTRA_MIDDLE_PROMOTION_LINE_X_OFFSET;
				pLineInst.Top:SetOffsetVal(adjustedXOffset, yOffset);
			elseif columnOffset == 2 then
				-- ↓→→
				local pLineInst:table = m_ReqLinesRightLongIM:GetInstance(pPromotionTreeInst.PromotionLinesContainer);
				local adjustedXOffset:number = x ~= 2 and xOffset or xOffset + EXTRA_MIDDLE_PROMOTION_LINE_X_OFFSET;
				pLineInst.Top:SetOffsetVal(adjustedXOffset, yOffset);
			elseif columnOffset == -2 then
				-- ←←↓
				local pLineInst:table = m_ReqLinesLeftLongIM:GetInstance(pPromotionTreeInst.PromotionLinesContainer);
				local adjustedXOffset:number = x ~= 2 and xOffset or xOffset - EXTRA_MIDDLE_PROMOTION_LINE_X_OFFSET;
				pLineInst.Top:SetOffsetVal(adjustedXOffset, yOffset);
			end
		end
...
-- ===========================================================================
function Initialize()
  print("HD GovernorDetailsPanel Override");
