...
function AddGovernorCandidate(governorDef:table, canAppoint:boolean)
	...
	for promotion in GameInfo.GovernorPromotionSets() do
		if promotion.GovernorType == governorDef.GovernorType then
			...
			-- HD 给总督升级排序
			local sortIndex = promotionDef.Level * 10 + promotionDef.Column;
			table.insert(promotionlist, {
				PromotionDef = promotionDef,
				SortIndex = sortIndex
			});

			-- AddPromotionInstance(governorInstance, governorDef, promotionDef, promotionDef.BaseAbility);
		end
	end
	...
	for _, data in ipairs(promotionlist) do
		AddPromotionInstance(governorInstance, governorDef, data.PromotionDef, data.PromotionDef.BaseAbility);
	end
...
-- ===========================================================================
function Initialize()
  print("HD GovernorPanel Override");
