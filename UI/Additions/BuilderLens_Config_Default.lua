...
table.insert(g_ModLenses_Builder_Config[m_BuilderLens_P1],
    function(pPlot)
        ...
        if pPlot:GetOwner() == localPlayer and not plotHasDistrict(pPlot) then
            if playerHasDiscoveredResource(pPlayer, pPlot) and not plotHasImprovement(pPlot) then
...
-- PILLAGED / UA
--------------------------------------
table.insert(g_ModLenses_Builder_Config[m_BuilderLens_PD],
    function(pPlot)
        local localPlayer = Game.GetLocalPlayer()
        if pPlot:GetOwner() == localPlayer and not plotHasDistrict(pPlot) then
            if plotHasImprovement(pPlot) and pPlot:IsImprovementPillaged() then
                return m_BuilderLens_PD
            end
        end
        return -1
    end)
